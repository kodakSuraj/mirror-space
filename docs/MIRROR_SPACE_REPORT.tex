...
%  Course: Product Development Practice -  ID351
...
% COLOURS
%------------------------------------------------------------
\definecolor{iiitkblue}{RGB}{40, 40, 40}
\definecolor{iiitkgold}{RGB}{90, 90, 90}
\definecolor{lightblue}{RGB}{244, 244, 244}
\definecolor{lightgold}{RGB}{238, 238, 238}
\definecolor{darkgray}{RGB}{32, 32, 32}
\definecolor{medgray}{RGB}{120, 120, 120}
\definecolor{boxbg}{RGB}{248, 248, 248}
\definecolor{defbg}{RGB}{245, 245, 245}
\definecolor{thmbg}{RGB}{242, 242, 242}
...
\fancyhead[C]{\small\color{iiitkblue}\textit{Product Development Practice -  ID351}}
...
\begin{center}
  {\fontsize{18}{22}\selectfont\bfseries\scshape\color{iiitkblue}
...
\begin{center}
  \IfFileExists{iiitklogo.jpg}{%
    {\includegraphics[height=4.8cm]{iiitklogo.jpg}}
  }{%
    \begin{tcolorbox}[colback=lightblue, colframe=iiitkblue, width=0.55\textwidth]
      \centering\bfseries IIITDM Kurnool Logo
    \end{tcolorbox}
  }
\end{center}
...
\begin{center}
  {\fontsize{14}{18}\selectfont\bfseries\color{darkgray}PROJECT SUBMISSION REPORT}\\[6pt]
  \begin{tcolorbox}[
    enhanced,
    width=\textwidth,
    colback=iiitkblue,
...
\begin{tcolorbox}[
  ...
  arc=4pt,
  boxrule=1pt,
  title={\centering\bfseries Submitted By},
  coltitle=white,
  colbacktitle=iiitkgold,
  left=10pt,
  right=10pt,
  top=6pt,
  bottom=6pt
...
\begin{tabularx}{\textwidth}{>{\bfseries\color{darkgray}}X >{\raggedleft\arraybackslash\color{iiitkblue}\bfseries}X}
J V Kousthub & 123CS0074 \\
Y Sri Rama Bharadwaj & 123CS0030 \\
K Suraj & 123AD0041 \\
S Sai Videsh & 523CS0013 \\
\end{tabularx}
\end{tcolorbox}
...
\begin{center}
  {\large\bfseries\color{iiitkblue}IIITDM Kurnool}\\[2pt]
  {\normalsize\color{darkgray}April 2026}
\end{center}
...
Real-world application examples are needed to validate the usefulness of low-latency local streaming in practical environments.
...
\begin{itemize}[leftmargin=*, itemsep=4pt]
  \item \textbf{Kousthub} (\texttt{Kousthub J V}): 12 commits in total under two author labels.
...
\subsection{Bharadwaj - Region Capture, Specific Window, Sensitivity Tuning}
Bharadwaj's contributions concentrated on region capture, specific-window capture, and practical sensitivity tuning. Delivered work includes:
\begin{enumerate}[label=\textbf{\arabic*.}, itemsep=6pt]
  \item \textbf{Region capture support}: Added and refined region-targeted capture capability through dedicated region-selection logic.
  \item \textbf{Specific-window capture support}: Stabilized specific-window targeting for practical, repeatable capture workflows.
  \item \textbf{Sensitivity tuning updates}: Tuned sensitivity handling to improve responsiveness for selective capture scenarios.
...
\subsection{Suraj - Fanout Architecture and PyPI Packaging}
Suraj architected a \textbf{one-to-many broadcasting model} enabling a single broadcaster to fan out simultaneously to multiple receivers across the network, each authenticated via a cryptographically generated session access ID with automatic stale-connection eviction.

He also packaged and published the complete system as a pip-installable Python library (\texttt{mirror-space}) on PyPI, including CLI entry points, a public encoder/decoder API, and end-to-end documentation.
...
Future work can target the following enhancements while preserving the same latency-first philosophy:
\begin{itemize}[leftmargin=*, itemsep=4pt]
  \item Transport security through DTLS or equivalent encrypted channel support.
  \item Multi-receiver fairness control for stable fanout under mixed network conditions.
  \item Optional remote-control and audio channels for richer collaboration workflows.
  \item Hardware-accelerated codec integration as an optional high-performance mode.
  \item Automated benchmark mode for standardized p50/p95/p99 latency reporting.
\end{itemize}
